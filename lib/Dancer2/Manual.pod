...
This can be replaced by using L<Path::Tiny> directly, which is what this DSL
will effectively do:

    use Path::Tiny qw< path >;
    my $dir = path($some_path)->parent->stringify;
...
This can also be done cleanly using L<Path::Tiny>, which is what this will
effectively do:

    use Path::Tiny qw< path >;
    path($0)->sibling( qw< lib File.pm > );
