...
=head3 Example 1: Basic Usage
    ...
    get 'animals' => '/zoo-kingdom/animals' => sub {
        return "Visit the animals at " . uri_for_route('animals');
    };
...
Parameters are how visitors of Danceyland communicate information to park staff.
...
TODO: more examples from existing manual
...
To organize your routes more effectively in Danceyland, you can use the
C<prefix> DSL keyword to group related routes under specific sections of
the park.

C<prefix> helps you to grow Danceyland as well. You can create additional
application modules that represent different areas of the park, each of
which is declared with its own C<prefix>.

=head3 Example: Using C<prefix> in Danceyland

Imagine Danceyland has different sections, like the I<Zoo Kingdom> and
I<Actionland>. You can use C<prefix> to organize these areas:

    package MyApp;
    use Dancer2;

    # Prefix for Zoo Kingdom
    prefix '/zoo-kingdom' => sub {
        get '/animals' => sub {
            return "Welcome to the Zoo Kingdom! Here you can find all the animals.";
        };

        get '/feed-schedule' => sub {
            return "Feeding Schedule: 10 AM and 4 PM.";
        };
    };
...
This example organizes routes by grouping all animal-related activities
under C</zoo-kingdom> and all rides under C</actionland>.
...
Think of templates like mad lib games you played as a kid. You just need
...
If you have built a single-page application, or other construct where
it is unnecessary to return a full HTML page, disabling a layout will
prevent Dancer2 from wrapping your page content. If you are building
modern interfaces with toolkits like L<htmx|https://htmx.org/> or
L<Turbo|https://turbo.hotwired.dev/>, it's important to only return
your page snippet.

If you need a route handler to return JSON, XML, or other serialized
data, you should look ahead to the L</Serialization> section. There
are special keywords and functionality built into Dancer2 to assist
with this that are better suited than the C<template> keyword.
...
that's it. The next time the client makes a request of the server, it
remembers nothing about the last time this client made a request, and so
all of that information must be sent again - the server has no concept
of persistence or "state" - this is left up to the developer to
implement.
...
data in memory, and sets the session to expire in one hour. Expiration
times are always set in seconds.
...
=head2 Growing your apps with appname
...
=head2 Running multiple apps with Plack::Builder
...
=head2 Async/Streaming/Delayed Responses
