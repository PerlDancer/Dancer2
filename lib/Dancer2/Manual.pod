...
parameters; these will be parameters supplied a) on the query string, b) within
the path itself (with named placeholders) and c), for HTTP POST requests, the
...
We recommend you use these instead of C<params>, and consider them much safer
(see L<Dancer2::Manual/params> for details).
...
If you want use redirect from a mounted package to root uri it's helpful to use
C<Dancer2::Plugin::RootURIFor> with this plugin it's possible like the following
sample:
