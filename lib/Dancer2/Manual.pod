...
C<encode_json> expects character data and returns UTF-8 encoded bytes.
...
You likely want to use C<encode_json> instead. Both return UTF-8 encoded
bytes, but C<encode_json> makes the intent explicit.
...
C<from_json> also decodes JSON to a Perl data structure. The input is
expected to be UTF-8 encoded bytes.
