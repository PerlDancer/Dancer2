...
through the specific C<route_parameters>, C<query_parameters>, and C<body_parameters> keywords, which return an L<Hash::MultiValue> object. This hashref is a
...
Tokens can be optional, for example:
    ...
    get '/hello/:name?' => sub {
        "Hello there, " .
          (defined route_parameters->get('name') ? route_parameters->get('name') : "whoever you are!");
    };
...
This is done with the B<pass> keyword, like in the following example
    ...
    get '/say/:word' => sub {
        return pass if (route_parameters->get('word') =~ /^\d+$/);
...
(note that for convenience, the request, session, route_parameters, body_parameters, query_parameters, and vars are
automatically accessible in the view, named C<request>, C<session>, C<params>, and C<vars>) - for example:
