...
=item PATCH: Visitors update their season patch information.
...
Importing C<Dancer2> automatically activates C<strict>, C<warnings> and
C<utf8> pragmas in your webapp, so your script doesn't need to.

Next, you define one of more route handlers (often called routes). A route
handler is made up of the following elements:

=over

=item * An HTTP Method (for example, get or post)

=item * A path pattern, which specifies a URL path this route handles
