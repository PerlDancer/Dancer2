...
through the specific C<route_parameters>, C<query_parameters>,
and C<body_parameters> keywords, which return an L<hashref>
(L<hash> in list context).
...
a L<hashref> result from the three different parameters.
