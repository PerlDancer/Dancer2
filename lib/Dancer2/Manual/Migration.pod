...
now. You can use an environment variable, however:
    ...
    export DANCER_CONFDIR=$PROJECT_ROOT/etc
...
The following Dancer 1 hooks have been renamed in Dancer2:

=over 4

=item * C<before_error_init> changed to C<init_error>.

=item * C<before_error_render> changed to C<before_error>.

=item * C<after_error_render> changed to C<after_error>.

=back

The original names are still supported, but you should change these to
new versions during your migration as the aliases may be removed in a
newer version of Dancer2.
