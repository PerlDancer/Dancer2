...
In Dancer 1, If you wanted to use Template::Toolkit instead of the built-in simple templating
...
the simple template engine, <% and %>, respectively.
...
In Dancer 2, the simple templating engine has been removed, and Template::Tiny is used as
the default templating engine instead. The default start_tag and end_tag are now [% and %],
so if you used the default in Dancer 1, you will have to explicitly change the start_tag and end_tag
values.

You can still enable Template::Toolkit with the same configuration option as in Dancer 1:
