...
Whereas in Dancer1 it was acceptable to write:

    template '/foo/bar'

In Dancer2, the leading slash is not permitted:
...
in your C<config.yml> file. Dancer2 will not respect this. Instead, invoke
middleware in your C<app.psgi> as demonstrated in
...
Keywords could be called explicitly:
...
Dancer2's DSL is implemented differently, and neither of these are possible
directly. If you need this ability, you can write a package which provides
this as follows:
...
This keyword does not exist in Dancer2.
