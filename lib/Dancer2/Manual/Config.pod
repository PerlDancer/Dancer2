...
=head1 USING ALTERNATE CONFIG READERS

Dancer2 allows you to use your own configuration system too. See the
L<Dancer2::ConfigReader> documentation for more information.

=head2 Bootstrapping other config readers

You can use the default Dancer2 configuration reader to load additional
...
See the L<Dancer2::ConfigReader> documentation for more information and
examples.
...
Default value is C<UTF-8>. HTTP responses without charset will be interpreted
as ISO-8859-1 by most clients.

If C<charset> is empty, Dancer2 will not encode response bodies. Handlers
should return already-encoded byte strings.
...
Serialized JSON responses are UTF-8 encoded by the JSON serializer. Pass
character strings to the serializer so it can do the encoding once.

=head3 strict_utf8 (boolean)

Controls how invalid UTF-8 bytes are handled at decoding boundaries (request
path/parameters) and in JSON serialization.

=over

=item * If false (default), invalid UTF-8 is left as bytes and a warning is
emitted.

=item * If true, invalid UTF-8 causes an immediate error.
...
C<views> can only be set once, preferably via config. Once the template
engine is initialized, changing the views setting has no effect.
...
=head3 strict_config_allow (array)

List of top-level configuration keys that should be ignored by strict config
warnings. This allows you to keep C<strict_config> enabled while permitting
...
If set to true, Dancer2 will B<not> add an C<X-Powered-By> header, and it
will avoid setting the "Server" header when using Dancer2's standalone
server.
...
=head3 DANCER_CONFIG_READERS

Specifies which configuration reader classes can be used to load
Dancer2 configuration.

=head3 DANCER_CONFDIR

Sets the configuration directory. This is only guaranteed to work with
the default config reader.

=head3 DANCER_CONFIG_VERBOSE

Outputs a lot of debugging information when reading the configuration of
the application. This is only guaranteed to work with the default config
reader.
...
This correlates to the C<envdir> config option. This is only guaranteed
to work with the default config reader.
