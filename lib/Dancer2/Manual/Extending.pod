...
Engines can live in the Dancer2 namespace:

    set serializer => 'JSON';
    set template => 'Mason';

(these resolve to L<Dancer2::Serializer::JSON> and L<Dancer2::Template::Mason>
respectively)

Or you can provide the fully qualified namespace using the C<+> indicator:

    set serializer => `+My::Awesome::Serializer`;
    set template => `+My::Template::DWIM`;

The latter is similar to how L<Plack> handles this.
