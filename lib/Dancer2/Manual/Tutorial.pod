...
Setting C<no_server_tokens> prevents Dancer2 from adding the
C<X-Powered-By> header and, when using Dancer2's standalone server, from
setting the C<Server> header to the Dancer2 version.

Does not touch the C<Server> header when running under an external PSGI
server.
