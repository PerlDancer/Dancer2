...
C<on_handler_exception> is called when an exception has been caught, at the
handler level, just before creating and rendering L<Dancer2::Core::Error>.
...
route level, just before rethrowing it higher. This hook receives the
error as argument.
