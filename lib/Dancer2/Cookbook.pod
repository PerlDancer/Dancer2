...
The 'appname=>"myapp"' assigns myapp::admin to the same app, called "myapp" after the 
orginating file. Without this directive, Dancer2 
splits the app into two different applications, each with their own namespace and 
variables. With different applications, please make sure that your path definitions 
do not overlap. Eg, when adding the default route as suggested above to package 
myapp, the /admin path can no longer be reached!
