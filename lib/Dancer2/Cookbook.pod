...
simple web app, but Dancer2 still has the power to work with larger projects.  
For the beginning, let's make an incredibly simple "Hello World" example:
...
script from L<App::Dancer2>, which will build the framework of your application 
with a single command:

    $ dancer2 -app mywebapp
    + mywebapp
    + mywebapp/config.yml
...
You may have to install the App::Dancer2 separately for this to work. For example: 
