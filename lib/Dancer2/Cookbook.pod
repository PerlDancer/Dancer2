...
Note that C<config-E<gt>{log}> should result undef error on a default scaffold since
...
like we do above with C<config-E<gt>{environment}='production'>. Of course, this value
